...
四个技能覆盖 选题 → 写作 → 标题 → 复盘。放在仓库内 `.agents/skills/`，
...
| 文件 | 作用 |
|---|---|
| `tools/preflight.py` | **发布前自检脚本**（V6 口径 + **公众号排版六项**）。只读、离线、确定性，退出码 0 = 可发布 |
...
```
今天有什么选题       → ai-content-ops 路由到 ai-topic-scout
选题3 / 写这篇       → ai-article-writer
起标题               → ai-title-optimizer
排版 / 准备发布      → ai-article-formatter
